...
class Utils
{
    ...
    public static <T> Option<T> first(List<T> src, FilterFn<T> filter)
    {
        for (T p : src)
            if (filter.apply(p))
                return new Some(p);
...
public class App
{
    public static final ActionFn<Person> PRINTIT = 
        new ActionFn<Person>() {
    ...
    public static void main(String[] args)
    {
        ...
        for (Person p : Utils.first(people, new FilterFn<Person>() {
            public boolean apply(Person it) { return it.getFirstName().equals("Farley"); }
        })) 
        {
